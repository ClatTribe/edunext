...
import useCollegeMicrositeComparison, {
  CompareBadge,
  CompareFloatingButton,
} from "../../../components/microsite/CollegeMicrositesComparison";
import CollegeBellButton from "../../../components/CollegeBellButton";
import {
  parseSearchQuery,
  applyBudgetFilter,
  doesCollegeMatchCourse,
  doesCollegeMatchLocation,
} from "../../../utils/data";
...
function isRankingArray(ranking: any): ranking is RankingEntry[] {
  return (
    Array.isArray(ranking) && ranking.length > 0 && "heading" in ranking[0]
  );
}
...
function getRankingsByCategory(
  ...
  for (const heading of RANKING_PRIORITY) {
    ...
    if (entry) {
      const matchingRow = entry.rows?.find(
        (row: RankingRow) => row.category === category,
      );
...
function getBestRanking(
  ...
  for (const heading of RANKING_PRIORITY) {
    const entry = map.get(heading);
    if (entry) {
      const bestRow = entry.rows.reduce(
        (best: RankingRow, row: RankingRow) =>
...
const CollegeMicrositesPage: React.FC = () => {
    ...
    if (wasRefresh === "true") {
      ...
      try {
        ...
        if (saved) {
          ...
          if (s.selectedCategory !== undefined)
            setSelectedCategory(s.selectedCategory);
          if (s.dynamicCategory !== undefined)
            setDynamicCategory(s.dynamicCategory);
  ...
  const { savedMicrositeCourses, toggleSavedMicrosite } =
    useSavedMicrositeCourses(user);
  ...
  const fetchColleges = async () => {
    try {
      ...
      // ★ Compute these BEFORE cacheKey so they're in scope everywhere
      const parsedForKey = searchQuery.trim()
        ? parseSearchQuery(searchQuery)
        : null;

      const allLocationsForKey = [
        ...new Set([
          ...(parsedForKey?.cities || []),
          ...(parsedForKey?.states || []),
      ...
      const allCoursesForKey = [
        ...new Set([
          ...(parsedForKey?.courses || []),
          ...manualFilters.courses,
        ]),
      ];
      const allExamsForKey = [
        ...new Set([
          ...(parsedForKey?.entranceExams || []),
          ...manualFilters.exams,
        ]),
      ];
      const searchTextForKey =
        parsedForKey?.searchText || manualFilters.collegeName || "";

      const hasOnlyCourses =
        allCoursesForKey.length > 0 &&
        !searchTextForKey &&
        allLocationsForKey.length === 0 &&
        allExamsForKey.length === 0;

      const resolvedRankingCat = hasOnlyCourses
        ? findRankingCategory(allCoursesForKey[0]) || allCoursesForKey[0]
        : null;

      // ★ cacheKey now has access to both variables
      const cacheKey = `colleges:search:${
        resolvedRankingCat
          ? `ranking:${resolvedRankingCat}`
          : searchQuery || "all"
      ...
      const {
        ...
        async () => {
          ...
          const allLocations = allLocationsForKey; // ★ reuse already computed
          const allCourses = allCoursesForKey;
          const allExams = allExamsForKey;
          const searchText = searchTextForKey;
          ...
          if (hasOnlyCourses && resolvedRankingCat) {
            // Pure course search → filter by ranking category (same as clicking the tab)
            query = query.or(
              `card_detail->>ranking.ilike.%${resolvedRankingCat}%`,
            );
          } else if (
            !searchText &&
            ...
            // All other searches → normal filter logic
            ...
            if (allCourses.length > 0) {
              const courseConditions = allCourses.map(
                (course) =>
            ...
            if (allExams.length > 0) {
              const examConditions = allExams.map(
                (exam) => `card_detail->>entrance_exam.ilike.%${exam}%`,
              );
  ...
  const paginatedColleges = useMemo(() => {
    const pageSlice = sortedColleges.slice(
      currentPage * perPage,
  ...
  const getMatchBadge = (course: College) => {
    ...
    if (score >= 90) {
      return (
        <span
          className="text-xs px-2 sm:px-3 py-1 rounded-full font-semibold flex items-center gap-1"
          style={{
  ...
  return (
    <DefaultLayout>
      <div
        className="flex-1 min-h-screen p-3 sm:p-4 md:p-6 mt-18 sm:mt-0"
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mb-4 sm:mb-6 md:mb-8">
            <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-1 sm:mb-2">
              ...
              <span className="text-[#F59E0B]">Perfect College</span>
            </h1>
          ...
          {error && (
            ...
            >
              ...
              <div>
                ...
                <p className="text-xs sm:text-sm" style={{ color: "#fecaca" }}>
                  {error}
                </p>
              </div>
            </div>
          ...
          {viewMode === "all" && availableCategories.size > 0 && (
            <div
              className="sticky top-0 z-40 mb-6 rounded-lg p-3 sm:p-4 backdrop-blur-xl overflow-x-auto"
              style={{
                backgroundColor: secondaryBg,
                border: `1px solid ${borderColor}`,
              }}
            >
              ...
              <p className="text-xs text-slate-500 mt-3">
                Colleges ranked by <strong>{selectedCategory}</strong> → NIRF,
                EduNext, QS (ranked colleges first)
              </p>
            </div>
          )}
          ...
          {loading ? (
            ...
            <div
              className="text-center py-12 sm:py-16 rounded-lg shadow-sm backdrop-blur-xl"
              style={{
                backgroundColor: secondaryBg,
                border: `1px solid ${borderColor}`,
              }}
                    ...
                    >
                      ...
                      {course.is_priority && (
                        <div className="mb-3 flex flex-wrap gap-2">
                          <span
                            className="inline-flex items-center gap-1.5 text-white text-xs font-bold px-2 sm:px-3 py-1 sm:py-1.5 rounded-full shadow-lg animate-pulse"
                            style={{
                              background:
                          ...
                          >
                            <Flame
                              size={12}
                              className="sm:w-3.5 sm:h-3.5 animate-bounce"
                            />
                          ...
                          >
                            <Star
                              size={12}
                              className="sm:w-3.5 sm:h-3.5 animate-spin"
                      ...
                      <div className="flex items-start justify-between mb-4 gap-3">
                        <div className="flex-1 min-w-0 pr-2">
                          ...
                          {rankings.length > 0 && (
                            <div className="mb-3">
                              ...
                              >
                                ...
                                <span>
                                  {rankings[0].source} {selectedCategory} #
                                  {rankings[0].value}
                                </span>
                              </span>
                            </div>
                            ...
                            <div className="flex items-center gap-2 mb-3">
                              <div className="flex items-center gap-1">
                                ...
                                <span className="text-sm font-bold text-white">
                                  {course["User Rating"]}/5
                                </span>
                              </div>
                              {course["User Reviews"] && (
                                <span className="text-xs text-slate-400">
                                  ({course["User Reviews"]} reviews)
                                </span>
                              )}
                            </div>
                          )}
                          ...
                          {viewMode === "recommended" && (
                            <div className="mt-2">{getMatchBadge(course)}</div>
                          )}
                        </div>
                        ...
                        >
                          <label
                            className="flex items-center gap-1.5 cursor-pointer group/compare hover:opacity-80 transition-opacity"
                      ...
                      <div className="space-y-3 sm:space-y-4">
                        {/* Fees Grid */}
                        <div
                          className="grid grid-cols-3 gap-3 sm:gap-4 pt-4 sm:pt-4"
                        ...
                        >
                          {course["Course Fees"] && (
                            <div className="group/info">
                              ...
                              <p className="font-bold text-white text-xs sm:text-sm break-words group-hover/info:text-amber-300 transition-colors">
                                {course["Course Fees"]}
                              </p>
                            </div>
                          )}
                        ...
                        <div
                          className="grid grid-cols-2 gap-3 sm:gap-4 pt-4 sm:pt-4"
                          style={{ borderTop: `1px solid ${borderColor}` }}
                        >
                          {course.scholarship && (
                          ...
                          {course.entrance_exam && (
                            <div className="group/card">
                              ...
                              >
                                <p className="text-xs text-slate-300 break-words font-medium">
                                  {course.entrance_exam}
                                </p>
                              </div>
                            </div>
                        ...
                        {/* CTA Buttons */}
                        <div
                          className="flex items-center gap-3 pt-4 sm:pt-4"
                          style={{ borderTop: `1px solid ${borderColor}` }}
                        >
                          {course.slug && (
                            <button
