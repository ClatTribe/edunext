...
import { Loader2, Quote, GraduationCap, FileText, ShieldCheck, Zap, CalendarDays } from "lucide-react"
...
/** Wrapper card per admission table block */
function AdmissionCard({ table, index }: { table: any; index: number }) {
  const title = getTitle(table, `Admission Table ${index + 1}`)
  ...
  return (
    <div className="space-y-10">
      ...
      <div className="space-y-3">
        ...
        {admissionTables.length > 0 && (
          <p className="text-slate-500 text-xs">
            {admissionTables.length} table{admissionTables.length > 1 ? 's' : ''} · Dates, eligibility & criteria
          </p>
        )}
      </div>
      ...
      {/* All admission tables */}
      {admissionTables.length > 0 ? (
        <div className="space-y-8">
          {admissionTables.map((table, index) => (
            <AdmissionCard key={index} table={table} index={index} />
          ))}
        </div>
      ) : (
        <div className="text-center py-20 rounded-[2rem] border border-dashed border-white/10 bg-white/[0.02]">
          <CalendarDays className="w-12 h-12 mx-auto mb-4 opacity-10" style={{ color: accentColor }} />
          <h3 className="text-sm font-bold text-white uppercase tracking-widest">No Admission Data</h3>
          <p className="text-slate-500 text-xs mt-1">Admission schedule is being updated.</p>
        </div>
      )}
    </div>
  )
