...
import { Loader2, CalendarDays, BarChart3, ChevronDown } from "lucide-react"
...
export default function AdmissionClient({ admissionData, uniqueHeadings }: AdmissionClientProps) {
  ...
  const visibleTables = admissionData.filter(item => (item.heading?.trim() || "General Information") === selectedHeading)
    ...
    <div className="space-y-8 max-w-7xl mx-auto px-4 pb-20">
      ...
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[9px] font-bold uppercase tracking-[0.3em] text-amber-500">Official Schedule</span>
        </div>
        <h1 className="text-2xl md:text-4xl font-black text-white uppercase tracking-tighter leading-tight">
          Selection <span className="text-amber-500">Protocol.</span>
        </h1>
      </div>
      ...
      >
        {visibleTables.length > 0 ? (
          visibleTables.map((table, index) => (
            <AdmissionCard key={index} table={table} />
          ))
        ) : (
