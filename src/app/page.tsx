...
import { Search, ChevronDown, Heart, User, BookOpen, DollarSign, Calendar, MapPin, Users, Home, GraduationCap, Award, Trophy, Building2, Filter } from 'lucide-react';

// --- Type Definitions for Data Structures ---
// Define types for better type safety, resolving the issues.
...
interface AdmitProfile {
  ...
  program: string;
  apps: number;
  avatar: 'S' | 'G'; // Limited to 'S' or 'G' based on sample
  verified: boolean;
}
...
const StudyAbroadPortal = () => {
  // Explicitly set the type for the state to resolve TypeScript issues
  const [activeSection, setActiveSection] = useState<'course-finder' | 'admit-finder' | 'scholarship-finder'>('course-finder');
  const [showVerifiedOnly, setShowVerifiedOnly] = useState(false);
  // Type is explicitly set to an array of numbers (the scholarship IDs)
  ...
  // Sample data for admit profiles
  ...
  // Fixes: Parameter 'scholarshipId' implicitly has an 'any' type. (7006)
  // Fixes: Argument of type 'any' is not assignable to parameter of type 'never'. (2345)
  // Fixes: Argument of type '(prev: never[]) => any[]' is not assignable to parameter of type 'SetStateAction<never[]>' (2345)
  ...
  const Sidebar = () => (
    <div className="w-64 bg-pink-50 h-screen p-4 border-r border-pink-200">
      ...
      <div className="flex items-center gap-2 mb-6">
        ...
        <div className="text-red-600 font-semibold">Ashish</div>
      </div>
