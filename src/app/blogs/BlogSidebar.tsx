...
export default function BlogSidebar({ latestBlogs, currentSlug, currentTitle }: BlogSidebarProps) {
  const [showCopyMessage, setShowCopyMessage] = useState(false);

  const handleShare = (platform: string) => {
    const url = `https://www.getedunext.com/blogs/${currentSlug}`;
    const title = `${currentTitle} | EduNext`;
    const description = `Check out this article: ${currentTitle}`;
                ...
                >
                  <div className="flex items-start gap-2.5">
                    ...
                    <div className="flex-1 min-w-0">
                      <h4
                        className="text-sm font-medium line-clamp-2 group-hover:underline transition-all"
                        style={{ color: '#E2E8F0' }}
                      >
                        {blog.title}
                      </h4>
        ...
        <div>
          ...
          <div className="flex justify-center items-center gap-2.5 flex-wrap">
            ...
            <button
              ...
              className="w-8 h-8 rounded-full flex items-center justify-center transition-all duration-300 hover:scale-110 relative"
              style={{ 
                backgroundColor: 'rgba(245, 158, 11, 0.1)',
                border: `1px solid ${borderColor}`
              }}
            ...
            >
              ...
              {showCopyMessage && (
                <span
                  className="absolute -top-8 left-1/2 transform -translate-x-1/2 text-xs font-medium px-2 py-1 rounded whitespace-nowrap"
                  style={{
                    backgroundColor: accentColor,
                    color: primaryBg,
                  }}
