...
function extractTopStreams(micrositeData: any): string[] {
  ...
  if (feeTables.length === 0) return []
  const foundStreams: Set<string> = new Set()
  for (const table of feeTables) {
    const heading = (table.heading || '').toLowerCase()
    const rows = table.rows || []
    for (const [stream] of Object.entries(STREAM_PRIORITY)) {
      const s = stream.toLowerCase()
      if (heading.includes(s)) foundStreams.add(stream)
      for (const row of rows) {
        const cell = cleanVal(row?.[0]).toLowerCase()
        if (cell.includes(s)) foundStreams.add(stream)
      }
    }
  }
  ...
  return feeTables.map(t => t.heading?.trim()).filter(Boolean).slice(0, 3)
}
...
function parseBudgetToLakhs(budgetArr: string[]): { min: number; max: number } {
  ...
  for (const b of budgetArr) {
    if (b === "Under 1L") { min = Math.min(min, 0); max = Math.min(max === Infinity ? 1 : max, 1) }
    if (b === "1-3L") { max = Math.max(max === Infinity ? 3 : max, 3); min = Math.min(min, 1) }
    if (b === "3-5L") { max = Math.max(max === Infinity ? 5 : max, 5) }
    if (b === "5L") { max = Math.max(max === Infinity ? 5 : max, 5) }
    if (b === "5-10L") { max = Math.max(max === Infinity ? 10 : max, 10) }
    if (b === "10-15L") { max = Math.max(max === Infinity ? 15 : max, 15) }
    if (b === "15-25L") { max = Math.max(max === Infinity ? 25 : max, 25) }
    if (b === "Above 15L" || b === "Above 25L") { max = Infinity }
  }
  return { min, max: max === Infinity ? 999 : max }
}

function parseFeeString(feeStr: string): number | null {
  if (!feeStr) return null
  const cleaned = feeStr.replace(/[₹,\s]/g, '').toLowerCase()
  const num = parseFloat(cleaned)
  if (isNaN(num)) return null
  if (cleaned.includes('lakh') || cleaned.includes('lac') || cleaned.includes('l')) return num
  if (cleaned.includes('crore') || cleaned.includes('cr')) return num * 100
  // If raw number > 10000, assume rupees → convert to lakhs
...
function computeMatch(profile: UserProfile, college: any, micrositeData: any): MatchResult {
  ...
  // 1. Stream Match (weight: 30)
  const weight1 = 30
  totalWeight += weight1
  const userDegree = (profile.degree || '').toLowerCase()
  const streamsLower = topStreams.map(s => s.toLowerCase())
  const streamMatch = streamsLower.some(s =>
    s.includes(userDegree) || userDegree.includes(s)
  )
  if (streamMatch) {
    weightedScore += weight1
    factors.push({ label: "Stream", score: 100, detail: `Offers ${profile.degree}`, positive: true })
  } else if (topStreams.length > 0) {
    weightedScore += weight1 * 0.15
    factors.push({ label: "Stream", score: 15, detail: `Known for ${topStreams[0]}, not ${profile.degree}`, positive: false })
  } else {
    weightedScore += weight1 * 0.5
    factors.push({ label: "Stream", score: 50, detail: "Stream data unavailable", positive: true })
  }

  // 2. Location Match (weight: 25)
  const weight2 = 25
  totalWeight += weight2
  const targetStates = profile.target_state.map(s => s.toLowerCase())
  const locationMatch = targetStates.some(s => collegeLocation.includes(s))
  if (locationMatch) {
    ...
    weightedScore += weight2 * 0.2
    factors.push({ label: "Location", score: 20, detail: `Not in ${profile.target_state.slice(0, 2).join(', ')}`, positive: false })
  }

  // 3. Budget Fit (weight: 25)
  const weight3 = 25
  totalWeight += weight3
  if (topFee && profile.budget.length > 0) {
    ...
    if (feeLakhs !== null) {
      if (feeLakhs <= max) {
        ...
        factors.push({ label: "Budget", score: 100, detail: `Fees ${topFee} within your range`, positive: true })
  ...
  // 4. Placement Strength (weight: 20)
  const weight4 = 20
  totalWeight += weight4
  if (placement.averagePackage || placement.highestPackage) {
    const avgFee = parseFeeString(placement.averagePackage || '')
    if (avgFee && avgFee >= 5) {
      weightedScore += weight4
      factors.push({ label: "Placement", score: 100, detail: `Avg package ${placement.averagePackage}`, positive: true })
    } else if (avgFee && avgFee >= 3) {
      weightedScore += weight4 * 0.7
      factors.push({ label: "Placement", score: 70, detail: `Avg package ${placement.averagePackage}`, positive: true })
    } else {
      weightedScore += weight4 * 0.4
      factors.push({ label: "Placement", score: 40, detail: `Avg package ${placement.averagePackage || 'low'}`, positive: false })
    }
  } else {
    weightedScore += weight4 * 0.3
    factors.push({ label: "Placement", score: 30, detail: "Placement data unavailable", positive: false })
  }
...
export default function CollegeMatchCard({
  ...
  // Fetch profile + compute match when dropdown opens and profile exists
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        // Compute match for current college
        const result = computeMatch(p, college, micrositeData)
        setMatchResult(result)

        // Always fetch recommended colleges — for any score
        ...
        if (targetStates.length > 0 && targetStates[0]) {
          query = query.ilike("location", `%${targetStates[0]}%`)
        }
        ...
        if (candidates && candidates.length > 0) {
          ...
          // Sort by score desc, exclude duplicates, take top 2
          const top = scored
      ...
      <div className="relative rounded-2xl border border-white/[0.06] bg-[#0a1022] overflow-hidden transition-all duration-500">
        ...
        <div className={`transition-all duration-500 ease-in-out overflow-hidden ${isOpen ? 'max-h-[1200px] opacity-100' : 'max-h-0 opacity-0'}`}>
          <div className="border-t border-white/[0.04] px-5 sm:px-6 pt-5 pb-6 space-y-5">
            ...
            {topStreams.length > 0 && (
              <div>
                <p className="text-[8px] font-bold uppercase tracking-widest text-slate-600 mb-2">Known For</p>
                ...
                <div className="p-4 sm:p-5">
                  ...
                  {matchResult && !loadingMatch && (
                    <div className="space-y-5">
                      ...
                      {/* Recommended Colleges — always shown */}
