...
export default function ResultsGrid({
  ...
  const totalAnswered = paddedAnswers.filter(
    (a) => a.selectedOption !== null,
  ).length;
  const totalUnanswered = EXPECTED_ANSWERED - totalAnswered; // how many of 180 are still blank
  const lowConf = paddedAnswers.filter(
    (a) => a.confidence === "low" && a.selectedOption !== null,
  ).length;
  ...
  const getSectionStats = (sectionAnswers: typeof paddedAnswers) => {
    ...
    const low = sectionAnswers.filter(
      (a) => a.confidence === "low" && a.selectedOption !== null,
    ).length;
    // Section B block for this subject (last 15 of the 50)
    const secBFilled = sectionAnswers
      .slice(35)
      .filter((a) => a.selectedOption !== null).length;
  ...
  return (
    ...
    >
      ...
      {sectionBWarnings.length > 0 && (
        <div
          className="mb-4 rounded-xl px-4 py-3 flex items-start gap-2"
          style={{
        ...
        >
          ...
          <div>
            <p className="text-red-400 text-sm font-semibold">
              Section B over-filled — bcz of photo quality or lighting:
            </p>
            <p className="text-red-300 text-xs mt-0.5">
              {sectionBWarnings.join(" · ")} — each Section B allows max 10
              answers
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-4 gap-3 mb-6">
        ...
        <div className="bg-[#050818] rounded-xl p-3 text-center">
          <p
            className={`text-2xl font-bold ${totalUnanswered > 0 ? "text-orange-400" : "text-green-400"}`}
          >
        ...
        <div className="bg-[#050818] rounded-xl p-3 text-center">
          <p
            className={`text-2xl font-bold ${lowConf > 0 ? "text-red-400" : "text-green-400"}`}
          >
            {lowConf}
          </p>
      ...
      <div className="flex gap-2 mb-4 overflow-x-auto pb-1">
        ...
        {sections.map((sectionAnswers, idx) => {
          ...
          const { answered, low, total, secBFilled, secBWarn } =
            getSectionStats(sectionAnswers);
          ...
          return (
            ...
            >
              ...
              <span
                className={`ml-1.5 text-xs ${isActive ? "text-black/70" : "text-slate-500"}`}
              >
                {start}–{end}
              </span>
              <span
                className={`ml-1 text-xs ${isActive ? "text-black/60" : "text-slate-600"}`}
              >
                {answered}/45
              </span>
              {secBWarn && (
      ...
      {activeSection === -1 ? (
        ...
        <div>
          <div className="flex items-center gap-3 mb-3">
            ...
            <span className="text-xs text-slate-600">
              Q{activeSection * SECTION_SIZE + 1}–
              {(activeSection + 1) * SECTION_SIZE}
            </span>
            <span className="text-xs text-slate-700">
              · Sec A: first 35 · Sec B: last 15 (10 filled)
            </span>
            <div className="flex-1 h-px bg-slate-800" />
          </div>
          <AnswerGrid
      ...
      <div className="mt-6 flex gap-4 justify-center flex-wrap text-xs text-slate-500">
        ...
        <span className="flex items-center gap-1">
          <span className="w-2.5 h-2.5 rounded bg-blue-900/50 border border-blue-500/30 inline-block" />
          Sec B slot
        </span>
        <span className="flex items-center gap-1">
            ...
            <div className="flex items-center justify-between px-4 py-2 border-b border-slate-800 flex-shrink-0">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="w-7 h-7 rounded-lg bg-slate-700 text-white hover:bg-slate-600 font-bold text-sm"
                >
                  −
                </button>
                ...
                <button
                  onClick={() => setZoom(1)}
                  className="text-xs text-slate-400 hover:text-white bg-slate-700 px-2 py-1 rounded-lg"
                >
                  Reset
                </button>
                <button
                  onClick={() => setLightboxOpen(false)}
                  className="text-white bg-red-600 hover:bg-red-500 px-3 py-1 rounded-lg text-xs font-medium"
                >
                  ✕
                </button>
              </div>
    ...
    <div className="space-y-2">
      ...
      <div className="flex items-center gap-2">
        ...
        <span className="text-[10px] text-blue-400/50">
          {
            sectionAnswers.slice(35).filter((a) => a.selectedOption !== null)
              .length
          }
          /10 filled
        </span>
  ...
  return (
    <div className="relative">
      <div
        onClick={() =>
          setEditingQ(
            editingQ === answer.questionNumber ? null : answer.questionNumber,
          )
        }
      ...
      >
        ...
        <span
          className={`font-bold text-sm leading-none ${isAnswered ? "text-white" : "text-slate-600"}`}
        >
          {answer.selectedOption || "—"}
        </span>
        <span
          className={`text-[8px] leading-none mt-0.5 transition-colors ${
            isEditing
              ? "text-amber-400"
              : "text-slate-700 group-hover:text-amber-500/70"
          }`}
        >
          ✎
        </span>
        ...
        >
          <div className="px-3 py-2 border-b border-slate-700 flex items-center justify-between gap-4">
            <p className="text-xs text-slate-400">
              ...
              {isSecB && (
                <span className="ml-1 text-blue-400 text-[10px]">Sec B</span>
              )}
            </p>
            <span className="text-xs text-slate-600">
              Now:{" "}
