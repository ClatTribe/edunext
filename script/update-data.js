...
// // ==================== CONFIGURATION ====================
// const JSON_FILE_PATH = './all_data_2.jsonl';
...
// ==================== CONFIGURATION ====================
const JSON_FILE_PATHS = ['./all_data_3.jsonl', './all_data_4.jsonl'];
...
async function updateColleges() {
  try {
    ...
    // 1. Read and parse multiple JSONL files
    const collegesData = JSON_FILE_PATHS.flatMap(filePath => {
    ...
    console.log(`✅ Found ${collegesData.length} colleges in total\n`);
    ...
    for (let i = 0; i < collegesData.length; i++) {
      const college = collegesData[i];
      const rawTitle = college.title || 'Unknown College';
      const collegeName = extractCollegeName(rawTitle);
      const slug = generateSlug(collegeName);
